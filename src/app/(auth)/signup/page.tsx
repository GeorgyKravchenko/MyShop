...
import { handleSignUp } from '@/lib/firebase/auth/signup';
import { useRouter } from 'next/navigation';
import { addData } from '@/lib/firebase/db/addData';
...
const SignupPage = () => {
  ...
  return (
    <div className="max-w-md mx-auto p-4 bg-gray-800 text-gray-100 rounded-lg shadow-xl my-10">
      <h1 className="text-3xl font-bold mb-6 text-center text-cyan-400">Реєстрація</h1>
      ...
      <form className="space-y-6" onSubmit={onSubmit}>
        ...
        <div>
          <label aria-label="Password" className="block mb-2 text-sm font-medium">
            Пароль
          </label>
          <input
            type="password"
          ...
          <ul className="flex gap-1 mt-2">
            {handlePasswordPower(watch('password')) >= 1 ? (
              <li className="h-1 w-full rounded-full bg-green-600"></li>
            ) : (
              <li className="h-1 w-full rounded-full bg-gray-600"></li>
            )}
            {handlePasswordPower(watch('password')) >= 2 ? (
              <li className="h-1 w-full rounded-full bg-green-600"></li>
            ) : (
              <li className="h-1 w-full rounded-full bg-gray-600"></li>
            )}
            {handlePasswordPower(watch('password')) >= 3 ? (
              <li className="h-1 w-full rounded-full bg-green-600"></li>
            ) : (
              <li className="h-1 w-full rounded-full bg-gray-600"></li>
            )}
            {handlePasswordPower(watch('password')) >= 4 ? (
              <li className="h-1 w-full rounded-full bg-green-600"></li>
            ) : (
              <li className="h-1 w-full rounded-full bg-gray-600"></li>
            )}
